...
class FinancialSummary:
    ...
    def display_summary(self):
        ...
        print("-" * 60)
        print(f"{'Job Name':<20} {'Client':<15} {'Hours':>8} {'Rate':>8} {'Total':>10}")
        print("-" * 60)
        
        for job in jobs:
            job_name, client_name, rate, hours, price = job
            clients.add(client_name)
            total_earnings += price
            total_hours += hours
            print(f"{job_name[:20]:<20} {client_name[:15]:<15} {hours:>8.1f} ${rate:>7.2f} ${price:>9.2f}")
